...
class RecruitersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create recruiter" do
    assert_difference("Recruiter.count") do
      post recruiters_url, params: { recruiter: { email: @recruiter.email, name: @recruiter.name } }
    end
  ...
  test "should update recruiter" do
    patch recruiter_url(@recruiter), params: { recruiter: { email: @recruiter.email, name: @recruiter.name } }
